...
for(var i in data){
	...
	row =  Alloy.createController('partials/locationRow', {data: data[i], index: i}).getView();
	locations.push(row);
	}else{
		row=Alloy.createController('partials/locationRow', {data:{title:'My Location Unavailable'}, index: i}).getView();
		locations.push(row);
	}
}
